...
const API_BASE_URL = 'https://your-api-domain.com/api'; // Replace with your API base URL

const API_ENDPOINTS = {
  GET_QUESTIONS: `${API_BASE_URL}/holland-questions`, // GET request to fetch all Holland questions
  SUBMIT_RESULTS: `${API_BASE_URL}/holland-results`, // POST request to submit test results
...
export const apiService = {
  ...
  async getQuestions(): Promise<QuestionResponse[]> {
    try {
      ...
      return new Promise(resolve => {
        setTimeout(() => resolve(MOCK_QUESTIONS), 500); // Simulate network delay
      });
    } catch (error) {
  ...
  async submitResults(data: SubmitResultRequest): Promise<ResultResponse> {
    try {
      ...
      // Mock response - remove this and uncomment above lines
      return new Promise(resolve => {
        setTimeout(() => resolve(MOCK_RESULT_RESPONSE), 1000); // Simulate network delay
      });
    } catch (error) {
